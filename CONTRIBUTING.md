# Contributing to ep_performance_test_hooks

Thanks for helping improve `ep_performance_test_hooks` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_performance_test_hooks` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_performance_test_hooks from your local clone
pnpm run plugins i --path ../ep_performance_test_hooks

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_performance_test_hooks/` are picked up after a server restart.
...
* Etherpad version, ep_performance_test_hooks version, browser + OS
* Steps to reproduce
