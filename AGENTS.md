# Agent Guide — ep_performance_test_hooks
...
`ep_performance_test_hooks` runs inside Etherpad's test harness. From an etherpad checkout that has installed this plugin via `pnpm run plugins i --path ../ep_performance_test_hooks`:
...
* Client: `postAceInit`, `documentReady`, `aceInitInnerdocbodyHead`, `aceInitialized`, `postToolbarInit`, `postTimesliderInit`
